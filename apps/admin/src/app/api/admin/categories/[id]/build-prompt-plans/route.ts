...
export async function POST(_request: Request, context: RouteContext) {
  try {
    ...
    const plans = buildPromptPlansFromCategory(category.name, ancestors, category.pose_prompt_specs);
    const result = await syncCategoryPosePromptImgSources(categoryId, plans, { replaceExisting: false });

    return NextResponse.json({ success: true, count: result.items.length });
  } catch (error) {
    const message = error instanceof Error ? error.message : "构建提示词计划失败。";
